...
void draw(){
  ...
  // 背景を描く
  tint(120, 120, 220);
  image(bgImg, 0, 0, sWidth, sHeight);
  noTint();

  // カメラがプレイヤーをなめらかに追いかけるようにする
  float leapY = 0.1f;
  if(player.y > stage.ch * stage.ChipSize) leapY = 0.01f;
  cameraX = lerp(cameraX, player.x - sWidth * 0.3f, 0.2f);
  cameraY = lerp(cameraY, player.y - sHeight * 0.7f, leapY);
  ...
  tint(255, 120);
  image(filterImg, 0, 0, sWidth, sHeight);
  noTint();
...
void drawYuki(){
  ...
  for(int i = 0; i < 100; i ++){
    ...
    //手前にあるほどカメラの動きに付いてくる
    float x = modulo(random(sWidth) -cameraX * (z - 1) + xt, sWidth);
    float y = modulo(random(sHeight) -cameraY * (z - 1) + yt, sHeight);
    ellipse(x, y, r, r);
  }
